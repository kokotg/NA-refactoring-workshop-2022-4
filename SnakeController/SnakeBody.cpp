...
SnakeBody::SnakeBody(IPort& displayPort, IPort& scorePort)
    :m_displayPort(displayPort), m_scorePort(scorePort) {}
...
void SnakeBody::removeTailSegmentIfNotScored(Segment const& newHead, std::pair<int, int> position)
{
    if (std::make_pair(newHead.x, newHead.y) == position) {
...
void SnakeBody::updateSegmentsIfSuccessfullMove(std::pair<int, int> position)
{
    ...
    if (isSegmentAtPosition(newHead.x, newHead.y) or isPositionOutsideMap(newHead.x, newHead.y)) {
        ...
        removeTailSegmentIfNotScored(newHead, std::pair<int, int> position);
    }
}
